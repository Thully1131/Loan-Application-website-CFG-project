...
const LoginPage = () => {
  ...
  const login = () => {
    ...
    Axios.post('http://www.localhost:3001/login', {
    ...
    <div className="Main-App">
      <div className='Sub-App'>
        <div>
          ...
          <div>
            <h1 text='login-pge' className='title'>Login Page</h1>
          </div>
          <div>
            <img src={require("../Pic/username.jpg")} alt="" className='user' />
            <input title="username" type="username" id="username" placeholder='username' className='name' onChange={handleChangeUsername} />
            <div className='otherInput'>
            </div>
            <img src={require("../Pic/password.jpg")} alt="" className='user' />
            <input title='password' type="password" id="password" placeholder='password' className='name' onChange={handleChangePassword} />
            <div className='otherInput'>
            </div>
            {errorMessage && <div className="error"> {errorMessage} </div>}
            <button title="LoginPageBtn" disabled={!isUserNameRegValid && !IsPasswordRegValid} onClick={e => usernameReg == "Admin" && passwordReg == "12345" ? managerLogin(e) : login(e)}>LogIn</button>
          </div>
